...
export default function VideoPlayer({
  ...
  const isLocalMuted = userMuteStates.get(userId) || false;
  const isRemoteMuted = userMuteStates.get(userData?.userId) || false;
      ...
      <div className="flex flex-col w-1/2 items-center justify-center m-4">
        <div className="flex items-center gap-2 mb-2">
          {user?._id === userData?.userId ? (
            <span className="font-medium">{userData.recipient}</span>
          ) : (
